...
SLOT="0/${PV}"
KEYWORDS="*"
IUSE="c++11 c++1y context debug doc icu mpi +nls python static-libs +threads tools"
RESTRICT="test"

RDEPEND="icu? ( >=dev-libs/icu-3.6:0=::${REPOSITORY}[c++1y(-)=] )
	icu? ( >=dev-libs/icu-3.6:0=::${REPOSITORY}[c++11(-)=] )
	...
	if use c++11; then
		if [[ $(tc-getCXX) == *g++* ]]; then
			if test-flag-CXX -std=gnu++11; then
			        use c++1y && test-flag-CXX -std=gnu++1y && append-cxxflags -std=gnu++1y || append-cxxflags -std=gnu++11
			else
			...
			if test-flag-CXX -std=c++11; then
				use c++1y && test-flag-CXX -std=c++1y && append-cxxflags -std=c++1y || append-cxxflags -std=c++11
			else
				die "C++11 not supported by currently used C++ compiler"
			fi
		fi
